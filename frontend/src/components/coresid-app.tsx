...
function AgentPromptCard({ coreAddress }: { coreAddress?: Address }) {
  const [copied, setCopied] = useState(false);

  const promptText = `Make a GET request to https://coresid.vercel.app/api/x402/mint?core=${coreAddress}`;
  ...
  return (
    <div className="mb-4 flex items-center justify-between gap-3 rounded-2xl border border-[var(--line)] bg-[var(--card)] p-3 backdrop-blur-xl">
      <div className="flex min-w-0 items-center gap-2">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 text-[var(--accent)]">
        ...
        <div>
          ...
          <p className="text-[10px] text-[var(--muted)]">
            Copy instructions and send them to your agent
          </p>
        </div>
      </div>
      <button
        type="button"
        onClick={handleCopy}
        className="flex h-8 flex-shrink-0 items-center gap-1.5 rounded-xl border border-[var(--line)] bg-[var(--btn-bg)] px-3 text-xs font-semibold text-[var(--ink)] transition-all hover:border-[var(--accent)] hover:text-[var(--accent)]"
      >
        {copied ? (
          "Copied!"
        ) : (
...
export function CoresIDApp() {
  ...
  async function ensureCorrectChain(overrideChainId?: number) {
    ...
    if (!ok) {
      setErrorMsg("Please switch your wallet to Base (chain 8453).");
    }
  }
        ...
        <div className="flex w-full max-w-sm flex-col mx-auto flex-1">
          ...
          {selectedRole === null && (
            <div className="flex-1 flex flex-col items-center justify-center text-center transition-opacity duration-300">
              <img src="/baselogomid.png" alt="CoresID" className="mb-3 w-40" />
    ...
    return (
      <main className="flex min-h-screen flex-col bg-[var(--surface)] px-4 py-6">
        ...
        <div className="flex w-full max-w-sm flex-col mx-auto flex-1">
          ...
          {level >= 0 ? (
            <div className="group relative mb-4 overflow-hidden rounded-[1.5rem] border border-[rgba(41,83,255,0.15)] bg-[var(--card)] shadow-[0_8px_40px_rgba(27,67,255,0.08)] transition-all">
              ...
              <button
                type="button"
                onClick={() => {
                  const text = `I have connected ${level} Seeds to my Core on CoresID\nYou can now link your EOAs and agents to your @baseapp account`;
          ...
          {/* AI Agent Prompt */}
          {slotsFree > 0 && <AgentPromptCard coreAddress={address} />}
